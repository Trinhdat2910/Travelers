...
use App\quocgia;
use Auth;
class KhachHangNVController extends Controller
{
    ...
    public function getDanhSachKH(){
    	...
    	return view('employee.qlKH.danhsach',['khachhang'=>$khachhang]);
    }
    public function getEditKH($MaKH){
    	$khachhang = khachhang::where('MaKH',$MaKH)->first();
    	return view('employee.qlKH.sua',['khachhang'=>$khachhang]);
    }
    public function SuaKH(Request $request, $MaKH){
    	...
    	$khachhang -> save();
    	return redirect('empLoyee/qlKH/danhsachKH') -> with('thongbao', 'Sửa Thành Công');
    }
    ...
    public function XoaKH($MaKH){
    	...
    	return redirect('employee/qlKH/danhsachKH')->with('thongbao', 'Bạn đã xóa Thành Công');
    }
}
