...
class DiaDiemNVController extends Controller
{
    public function getDanhSachDD(){
    	$diadiem = diadiemdulich::all();
    	return view('employee.qlDD.danhsach',['diadiemdulich'=>$diadiem]);
    }

    public function getThemDD(){
    	$tinh = tinh::all();
        
    	return view('employee.qlDD.them',['tinh'=>$tinh]);
    }
     ...
     public function postThemDD(Request $request ){
    	...
    	$diadiemdulich -> save();
    	return redirect('employee/qlDD/danhsachDD') -> with('thongbao', 'Thêm Thành Công');
    }
     public function getEditDD($MaDD){
        $diadiemdulich = diadiemdulich::find($MaDD);
        $tinh = tinh::all();
        return view('employee.qlDD.sua',['tinh'=>$tinh],['diadiemdulich'=>$diadiemdulich]);
    }
    ...
    public function XoaDD($MaDD){
        ...
        if($diadiem->delete()){
            return redirect('employee/qlDD/danhsachDD')->with('thongbao', 'Bạn đã xóa Thành Công');
        }
        else{
            return  redirect('employee/qlDD/danhsachDD')->with('thongbao', 'Bạn không được phép xóa');
        }
